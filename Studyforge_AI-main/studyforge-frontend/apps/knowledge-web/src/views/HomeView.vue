...
const contentTypes = computed(() => {
  if (preferencesStore.languageCode === 'en_US') {
    return [
      ...
      { code: 'GUIDE', name: 'Guide', icon: '📚' },
      { code: 'NOTE', name: 'Note', icon: '📝' },
      { code: 'REVIEW', name: 'Review', icon: '🔄' },
      { code: 'QUESTION', name: 'Question', icon: '❓' },
      { code: 'RESOURCE', name: 'Resource', icon: '🔗' },
      { code: 'ROADMAP', name: 'Roadmap', icon: '🗺️' },
      { code: 'MOMENT', name: 'Moment', icon: '💬' }
    ];
  }

  return [
    { code: 'ALL', name: '全部类型', icon: '📋' },
    { code: 'GUIDE', name: '攻略', icon: '📚' },
    { code: 'NOTE', name: '笔记', icon: '📝' },
    { code: 'REVIEW', name: '复盘', icon: '🔄' },
    { code: 'QUESTION', name: '求助', icon: '❓' },
    { code: 'RESOURCE', name: '资源', icon: '🔗' },
    { code: 'ROADMAP', name: '路线', icon: '🗺️' },
    { code: 'MOMENT', name: '动态', icon: '💬' }
  ];
});
...
const visiblePosts = computed(() => {
  ...
  return decoratedPosts.value.filter((post) => {
    ...
    const matchesContentType = activeContentType.value === 'ALL' || post.contentType === activeContentType.value;
    const matchesKeyword =
      !normalizedKeyword ||
      post.title.toLowerCase().includes(normalizedKeyword) ||
      post.summary.toLowerCase().includes(normalizedKeyword);

    return matchesCategory && matchesContentType && matchesKeyword;
  });
});
  ...
  <div class="knowledge-home">
    ...
    <section class="content-column">
      ...
      <div class="content-type-filter">
        <span class="filter-label">{{ copy.contentTypes }}:</span>
        <div class="type-chips">
          <button
            ...
            type="button"
            class="type-chip"
            :class="{ active: activeContentType === ct.code }"
            @click="activeContentType = ct.code"
          >
            <span class="chip-icon">{{ ct.icon }}</span>
            <span class="chip-name">{{ ct.name }}</span>
          </button>
        </div>
      </div>
