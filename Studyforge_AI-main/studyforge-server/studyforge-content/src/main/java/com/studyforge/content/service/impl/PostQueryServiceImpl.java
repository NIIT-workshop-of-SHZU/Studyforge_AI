...
public class PostQueryServiceImpl implements PostQueryService {
    ...
    public PostDetailVO getDetail(Long postId, String languageCode) {
        ...
        return new PostDetailVO(
                ...
                post.getContentType() == null ? "GUIDE" : post.getContentType(),
    ...
    @Override
    public List<PostSummaryVO> list(String languageCode, String categoryCode, String contentType, String keyword, int limit) {
        int normalizedLimit = normalizeLimit(limit, 30);
        return postMapper.selectPublished(emptyToNull(categoryCode), emptyToNull(contentType), emptyToNull(keyword), normalizedLimit)
                .stream()
                .map(this::toOriginalSummary)
                .filter(Objects::nonNull)
                .toList();
